...
const wallSpatialIndex = createWallSpatialIndex();
let walls = [];
let lightPanels = [];
let phonePositions = [];
let phoneMeshes = [];
...
let lastAudioProximityUpdate = 0;
let qualitySettings = null;
let chunkTracker = createChunkTracker();
let profileDisplayPrevTime = performance.now();
const profileStats = new Map();
const profileEnabled = new URLSearchParams(window.location.search).has('profile');

const CHUNK_ROTATION_THRESHOLD = 0.18;
const QUALITY_PRESETS = {
    desktop: {
        audioProximityInterval: 150,
        bloom: true,
        postEffects: true,
        preloadDist: 4,
        renderDist: 2,
...
function getQualitySettings(isMobile) {
    ...
    return isMobile ? QUALITY_PRESETS.mobile : QUALITY_PRESETS.desktop;
}
...
function shouldRefreshChunks() {
    ...
    return playerChunkX !== chunkTracker.chunkX ||
        ...
        angleDelta(camera.rotation.y, chunkTracker.yaw) >= CHUNK_ROTATION_THRESHOLD ||
        angleDelta(camera.rotation.x, chunkTracker.pitch) >= CHUNK_ROTATION_THRESHOLD;
}
...
function resetChunkTracker() {
    chunkTracker = createChunkTracker();
}

function startProfile() {
    return profileEnabled || debugMode ? performance.now() : 0;
}

function recordProfile(name, startTime) {
    if (startTime <= 0) {
        return;
    }

    const elapsed = performance.now() - startTime;
    const stat = profileStats.get(name) ?? { average: elapsed, samples: 0 };
    stat.average = stat.samples === 0 ? elapsed : stat.average * 0.9 + elapsed * 0.1;
    stat.samples++;
...
function handleCollision(target) {
    ...
    const wallsToCheck = queryWallsNearBox(wallSpatialIndex, collisionPlayerBounds, nearbyCollisionWalls);
    ...
    for (const wall of wallsToCheck) {
        ...
        if (collisionPlayerBounds.intersectsBox(collisionWallBounds)) {
            ...
            wallWorldPosition.copy(wall.userData.worldCenter);
...
function cacheDomElements() {
    ...
    profileValueElement = document.createElement('div');
    profileValueElement.style.display = 'none';
    fpsCounterElement.append(profileValueElement);
...
function applyRendererSize() {
    ...
    const pixelRatio = Math.min(window.devicePixelRatio * qualitySettings.renderScale, 1);
...
function clearWorldState() {
    ...
    chunkBorders = [];
    debugNormals = [];
    rebuildWallSpatialIndex(wallSpatialIndex, walls);
    resetChunkTracker();
}
...
function updateFpsCounter() {
    ...
    if (currentTime >= fpsPrevTime + 1000) {
        ...
        updateProfileDisplay(currentTime);
    }
}

function updateProfileDisplay(currentTime) {
    if (!profileValueElement) {
        return;
    }

    const shouldShowProfile = profileEnabled || debugMode;
    profileValueElement.style.display = shouldShowProfile ? 'block' : 'none';

    if (!shouldShowProfile || currentTime < profileDisplayPrevTime + 1000) {
        return;
    }

    profileValueElement.textContent = [...profileStats.entries()]
        .map(([name, stat]) => `${name}: ${stat.average.toFixed(1)}ms`)
        .join(' | ');
    profileDisplayPrevTime = currentTime;
}
...
function updateRuntimeSystems() {
    ...
    refreshChunks(false);
    updateAudioProximity(performance.now());

    updatePhoneInteractPrompt(nearestPhoneDist, isInteractingWithPhone);

    const entityStart = startProfile();
...
function refreshChunks(force) {
    if (!force && !shouldRefreshChunks()) {
        return;
    }
    ...
    markChunksRefreshed();
    recordProfile('chunk', chunkStart);
}

function updateAudioProximity(currentTime) {
    if (currentTime - lastAudioProximityUpdate < qualitySettings.audioProximityInterval) {
        return;
    }
